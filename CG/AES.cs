...
namespace CG
{
    public  class AES
    {
        ...
        public static byte[] AesCtrDecrypt(byte[] key, byte[] counter, byte[] data,string userString)
        {
            // this is SDBH way of telling if a file is encrypted, using exact same method.
            if (userString != "CRI_CFATTR:ENC1") 
                return data;

            // if the uString did equal the encryption note, then decrypt
